...
fn pixel_location_to_pointer_position(
    hwnd: HWND,
        ...
        match pointer_type {
            ...
            winuser::PT_PEN => {
                vec![handle_pen_info(
                    ...
                    phase,
                    &{
                        let mut pen_info = std::mem::MaybeUninit::uninit();
                        winuser::GetPointerPenInfo(pointer_id, pen_info.as_mut_ptr());
                        pen_info.assume_init()
                    },
                )]

                // Causes input to skip on a surface book 2? avoiding for now
                ...
                //     .iter()
                //     .map(|pen_info| {
                //         handle_pen_info(pointer_id, modifiers, dpi_factor, phase, pen_info)
                //     })
                //     .collect()
            }
...
fn handle_pen_info(
    ...
    dpi_factor: f64,
    phase: PointerPhase,
    pen_info: &winuser::POINTER_PEN_INFO,
) -> Pointer {
